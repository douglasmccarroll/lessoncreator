...
import com.brightworks.util.Utils_File;
    ...
    public class Fix_Lesson_IncorrectFileName_Xml extends Fix {
        public override function get humanReadableFixDescription():String {
            return "Rename file";
        }

        public function Fix_Lesson_IncorrectFileName_Xml() {
            super();
        }

        public override function fix(lessonDevFolder:LessonDevFolder, lessonProblem:LessonProblem):Boolean {
            var existingFile:File = Utils_File.getSingleFileInFolder(lessonDevFolder.getSubfolder_xml());
            var newFile:File = lessonDevFolder.folder.resolvePath(lessonDevFolder.lessonId + "." + Constants_Misc.FILE_NAME_EXTENSION__LESSON_XML_FILE);
            try {
